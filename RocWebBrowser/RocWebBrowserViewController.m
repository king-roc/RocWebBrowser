...
static NSString * HomeAddress = @"";
static void *RocWebBrowserContext = &RocWebBrowserContext;

@interface RocWebBrowserViewController () <UIAlertViewDelegate>
...
@implementation RocWebBrowserViewController

#pragma mark - Static Initializers

+ (RocWebBrowserViewController *)webBrowser {
    //Push Browser
    RocWebBrowserViewController *webBrowserViewController = [RocWebBrowserViewController webBrowserWithConfiguration:nil];
...
+ (UINavigationController *)navigationControllerWithWebBrowser {
    ...
    //Present Browser
    RocWebBrowserViewController *webBrowserViewController = [[self alloc] initWithConfiguration:nil];
    return [RocWebBrowserViewController navigationControllerWithBrowser:webBrowserViewController];
}

+ (RocWebBrowserViewController *)webBrowserWithConfiguration:(WKWebViewConfiguration *)configuration {
    RocWebBrowserViewController *webBrowserViewController = [[self alloc] initWithConfiguration:configuration];
    return webBrowserViewController;
}
...
+ (UINavigationController *)navigationControllerWithBrowser:(RocWebBrowserViewController *)webBrowser {
...
- (void)viewDidLoad {
    ...
    if(self.wkWebView) {
        ...
        [self.wkWebView addObserver:self forKeyPath:NSStringFromSelector(@selector(estimatedProgress)) options:0 context:RocWebBrowserContext];
        //kvo
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.navigationController setNavigationBarHidden:self.navHidden animated:YES];
...
- (void)updateToolbarState {
    ...
    if(self.wkWebView.loading || self.uiWebViewIsLoading) {
        barButtonItems = @[self.backButton,
                           self.flexibleSeparator,
                           self.forwardButton,
                           self.flexibleSeparator,
                           self.fixedSeparator,
                           self.flexibleSeparator,
...
- (void)setupToolbarItems {
    ...
    self.fixedSeparator = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"footer_nav01"] style:UIBarButtonItemStylePlain target:self action:@selector(Home:)];
...
-(void)Home:(id)sender{
        NSURL *URL = [NSURL URLWithString: HomeAddress];
        [self loadURL:URL];
}
...
@implementation UINavigationController(RocWebBrowser)

- (RocWebBrowserViewController *)rootWebBrowser {
    
    UIViewController *rootViewController = [self.viewControllers objectAtIndex:0];
    return (RocWebBrowserViewController *)rootViewController;
}
